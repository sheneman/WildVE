...
parser = argparse.ArgumentParser(prog='wildve', description='WildVE: Wildlife Video Extractor - Analyze videos using an ensemble of AI models and extract clips containing wildlife.')
...
def load_models(pid):
	...
	megadetector5_model = pw_detection.MegaDetectorV5(device=device, pretrained=True)
	...
	megadetector6v9_model = pw_detection.MegaDetectorV6(device=device, version="yolov9-mit-extra")

	print(f"PID={pid}: Loading Megadetector 6v10 model...")
	megadetector6v10_model = pw_detection.MegaDetectorV6(device=device, version="MDV6-yolov10-e")
	...
	return megadetector5_model, megadetector6v9_model, megadetector6v10_model, tiger_model, florence_model, florence_processor, clip_model, clip_processor
...
def parse_megadetector_detections(detections):
	...
	# Check if there are actual detections
	if detections is not None and hasattr(detections, 'confidence') and detections.confidence is not None:
		if len(detections.confidence) > 0:
			...
			megadetector_has_animal = any(cls == 0 for cls in megadetector_classes)
...
def ensemble_detection(img, megadetector5_model, megadetector6v9_model, megadetector6v10_model, tiger_model, florence_model, florence_processor, clip_model, clip_processor):


	# MegaDetector V5
	megadetector5_results    = megadetector5_model.single_image_detection(img)
	megadetector5_detections = megadetector5_results.get('detections', None)
	megadetector5_classes, megadetector5_confidences, megadetector5_has_animal, megadetector5_max_confidence = parse_megadetector_detections(megadetector5_detections)

	# MegaDetector 6v9
	megadetector6v9_results    = megadetector6v9_model.single_image_detection(img)
	megadetector6v9_detections = megadetector6v9_results.get('detections', None)
	megadetector6v9_classes, megadetector6v9_confidences, megadetector6v9_has_animal, megadetector6v9_max_confidence = parse_megadetector_detections(megadetector6v9_detections)

	# MegaDetector 6v10
	megadetector6v10_results    = megadetector6v10_model.single_image_detection(img)
	megadetector6v10_detections = megadetector6v10_results.get('detections', None)
	megadetector6v10_classes, megadetector6v10_confidences, megadetector6v10_has_animal, megadetector6v10_max_confidence = parse_megadetector_detections(megadetector6v10_detections)
	...
	similarity = image_features @ text_features.T  # shape: [1, 2]
	similarity = similarity.squeeze(0).cpu().numpy()

	clip_detection = similarity[0] > similarity[1]
	clip_confidence = similarity[0] if clip_detection else 0.0
...
def telemetry_log(log_fd, data):
	...
	# Filter to keep only int, float, and str
	filtered_data = {k: v for k, v in converted_data.items() if isinstance(v, (int, float, str))}

	# Add the filename to the filtered data dictionary
	filtered_data = {"chunk_idx": data.get("chunk_idx", -1), "filename": filename, **filtered_data}

	# Reorder the fieldnames to have 'filename' as second column
	fieldnames = list(filtered_data.keys())
	if "filename" in fieldnames and fieldnames[1] != "filename":
...
def get_video_chunk(invid, interval_sz, pu_lock, log_fd):
	global chunk_idx
	...
	with pu_lock:
		try:
			results = ensemble_detection(inference_frame, 
										 megadetector5_model, 
										 megadetector6v9_model, 
										 megadetector6v10_model, 
									     tiger_model, 
										 florence_model, 
										 florence_processor, 
										 clip_model, 
	...
	res = {
		...
		"clip_detection": results.get("clip_detection"),
		"clip_conf": results.get("clip_conf"),
	...
	total_models = 5
	detections = [
		results.get("megadetector5_detection"),
		results.get("megadetector6v9_detection"),
		results.get("megadetector6v10_detection"),
		results.get("tiger_detection"),
		results.get("florence_detection"),
		results.get("clip_detection")
	]
	
	confidences = [
		results.get("megadetector5_conf", 0) if results.get("megadetector5_detection") else 0,
		results.get("megadetector6v9_conf", 0) if results.get("megadetector6v9_detection") else 0,
		results.get("megadetector6v10_conf", 0) if results.get("megadetector6v10_detection") else 0,
		results.get("tiger_conf", 0) if results.get("tiger_detection") else 0,
		1 if results.get("florence_detection") else 0,
		results.get("clip_conf", 0) if results.get("clip_detection") else 0,
...
def process_chunk(pid, chunk, pu_lock, report_lock):
	global megadetector5_model, megadetector6v9_model, megadetector6v10_model, tiger_model, florence_model, florence_processor, clip_model, clip_processor
	global chunk_idx, most_recent_written_chunk

	# Load models only once per process
	megadetector5_model, megadetector6v9_model, megadetector6v10_model, tiger_model, florence_model, florence_processor, clip_model, clip_processor = load_models(pid)

	print(f"PID={pid}, CHUNK={chunk}")

	log_filename = os.path.join(args.logging, f"{pid}_telemetry.csv")
	log_fd = open(log_filename, "w")
	print(f"Opened telemetry log: {log_fd}")
	...
	for filename in chunk:
		...
		frame_chunk, success = get_video_chunk(invid, interval_frames, pu_lock, log_fd)
